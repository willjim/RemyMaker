...
/**
 * Export parsed 3DGS data back to a standard uncompressed binary PLY file.
 * @param {object} data - Reconstructed data from parser
 * @returns {ArrayBuffer}
 */
export function exportToPLY(data) {
  const { positions, colors, scales, rotations, opacities, count } = data;
  
  const SH_C0 = 0.28209479177387814;
  
  // Create standard 3DGS PLY header
  const header = `ply
...
end_header\n`;
  ...
  const vertexStride = 68; // 17 floats
  const totalLength = headerBytes.byteLength + count * vertexStride;
  
  const buffer = new ArrayBuffer(totalLength);
  const view = new DataView(buffer);
  
  // Write header
  const headerUint8 = new Uint8Array(buffer, 0, headerBytes.byteLength);
  headerUint8.set(headerBytes);
  ...
  for (let i = 0; i < count; i++) {
    ...
    // f_dc_0, f_dc_1, f_dc_2 (convert RGB colors back to SH coefficients)
    const r = colors[i * 3];
    const g = colors[i * 3 + 1];
    const b = colors[i * 3 + 2];
    view.setFloat32(offset + 24, (r - 0.5) / SH_C0, true);
    view.setFloat32(offset + 28, (g - 0.5) / SH_C0, true);
    view.setFloat32(offset + 32, (b - 0.5) / SH_C0, true);
    
    // Opacity (convert sigmoid-activated 0-1 back to logit)
    const op = Math.max(0.0001, Math.min(0.9999, opacities[i]));
    const logitOpacity = Math.log(op / (1.0 - op));
    view.setFloat32(offset + 36, logitOpacity, true);
    
    // Scales (convert raw scale back to log-scale)
    const sx = Math.max(1e-5, scales[i * 3]);
    const sy = Math.max(1e-5, scales[i * 3 + 1]);
    const sz = Math.max(1e-5, scales[i * 3 + 2]);
    view.setFloat32(offset + 40, Math.log(sx), true);
    view.setFloat32(offset + 44, Math.log(sy), true);
    view.setFloat32(offset + 48, Math.log(sz), true);
    
    // Rotations (w, x, y, z)
    view.setFloat32(offset + 52, rotations[i * 4], true);     // w (rot_0)
    view.setFloat32(offset + 56, rotations[i * 4 + 1], true); // x (rot_1)
    view.setFloat32(offset + 60, rotations[i * 4 + 2], true); // y (rot_2)
    view.setFloat32(offset + 64, rotations[i * 4 + 3], true); // z (rot_3)
